		...
		if courses:
			for course in courses:
				...
				if new_start and new_start != old_start:
					changed = True
					dates.set(ntiid, 'available_for_submission_beginning', new_start)
					event_notify(QAssessmentDateContextModified(dates, ntiid, 'available_for_submission_beginning'))
				if new_end and new_end != old_end:
					changed = True
					dates.set(ntiid, 'available_for_submission_ending', new_end)
					event_notify(QAssessmentDateContextModified(dates, ntiid, 'available_for_submission_ending'))
				if changed:
