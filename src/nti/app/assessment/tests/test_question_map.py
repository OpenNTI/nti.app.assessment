        ...
        "tag:nextthought.com,2011-10:mathcounts-HTML-mathcounts2012.mathcounts_2011_2012": {
            ...
            "Items": {
                "tag:nextthought.com,2011-10:mathcounts-HTML-mathcounts2012.warm_up_1": {
                    "AssessmentItems": {
                        "tag:nextthought.com,2011-10:mathcounts-NAQ-mathcounts2012.naq.qid.1": """ + \
    json.dumps(ASSM_ITEMS['tag:nextthought.com,2011-10:testing-NAQ-temp.naq.testquestion']) + """
                    },
...
class TestQuestionMap(AssessmentLayerTest):
    ...
    def setUp(self):
        ...
        component.getGlobalSiteManager().registerUtility(
            self, IGlobalContentPackageLibrary)
    ...
    def _do_test_create_question_map_captures_set_ntiids(self, index_string=ASSM_STRING_W_SET):
        ...
        populate_question_map_from_text(question_map,
                                        index_string,
        ...
        for item in assm_items:
            if IQuestion.providedBy(item):
                question = item
                assert_that(item,
                            has_property('ntiid', 'tag:nextthought.com,2011-10:testing-NAQ-temp.naq.testquestion'))
                assert_that(item,
                            has_property('__name__', 'tag:nextthought.com,2011-10:testing-NAQ-temp.naq.testquestion'))
            elif IQuestionSet.providedBy(item):
                qset = item
                assert_that(item,
                            has_property('ntiid', 'tag:nextthought.com,2011-10:testing-NAQ-temp.naq.set.testset'))
                assert_that(item,
                            has_property('__name__', 'tag:nextthought.com,2011-10:testing-NAQ-temp.naq.set.testset'))

        qset_question = qset.questions[0]
        assert_that(qset_question, is_(question))
        assert_that(qset_question,
                    has_property('ntiid',     'tag:nextthought.com,2011-10:testing-NAQ-temp.naq.testquestion'))
        assert_that(qset_question,
                    has_property('__name__',  'tag:nextthought.com,2011-10:testing-NAQ-temp.naq.testquestion'))
        ...
        for question in question_set.questions:
            assert_that(question,
                        is_(same_instance(component.getUtility(IQuestion, name=question.ntiid))))
    ...
    def test_create_from_mathcounts2012_no_Question_section_in_chapter(self):
        ...
        populate_question_map_from_text(question_map,
                                        index_string, MockEntry())

        assert_that(question_map, has_length(1))

        ntiid = 'tag_nextthought_com_2011-10_mathcounts-HTML-mathcounts2012_warm_up_1.html'
        assm_items = question_map.by_file.get(ntiid)

        assert_that(assm_items, has_length(1))
        question = assm_items[0]

        assert_that(question,
                    has_property('__name__', 'tag:nextthought.com,2011-10:mathcounts-NAQ-mathcounts2012.naq.qid.1'))
        assert_that(question,
                    has_property('ntiid', 'tag:nextthought.com,2011-10:mathcounts-NAQ-mathcounts2012.naq.qid.1', ))
    ...
    def test_create_with_assignment(self):
        ...
        asg = component.getUtility(IQAssignment,
                                   name='tag:nextthought.com,2011-10:testing-NAQ-temp.naq.asg.assignment')
        qset = component.getUtility(IQuestionSet,
                                    name='tag:nextthought.com,2011-10:testing-NAQ-temp.naq.set.set')
        q = component.getUtility(IQuestion,
                                 name='tag:nextthought.com,2011-10:testing-NAQ-temp.naq.testquestion')
