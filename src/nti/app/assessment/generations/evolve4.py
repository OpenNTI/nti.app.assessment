...
def do_evolve(context):
	...
	with site(ds_folder):
		...
		# Drop old catalog
		assess_catalog = lsm.queryUtility( IMetadataCatalog, name=CATALOG_NAME )
		lsm.unregisterUtility( assess_catalog, provided=IMetadataCatalog )
		intids = component.getUtility( IIntIds )
		intids.unregister( assess_catalog )

	logger.info( 'Finished app assessment evolve (%s)', generation )

def evolve(context):
	"""
	Evolve to generation 4 by moving assessment catalog to
	library catalog.
	"""
	do_evolve(context)
